...
public class DynamicMethodInvoker {
	...
	private String getBeanName(String serviceName, BeanType type) {
	...
	private Object getBean(String serviceName, BeanType type) {
		String beanName = getBeanName(serviceName, type);
		
		return applicationContext.getBean(beanName);
	}
	
	private boolean isMethodExists(Object bean, String methodName, Class<?> paramType) {
	...
	public <T> Map<String, Object> invokeServiceMethod(String serviceName, MethodType prefix, String urlid, Class<T> paramType, T paramValue) {
		...
		try {
			...
			// 서비스에 메서드 존재하는지 조회
			Object service = getBean(serviceName, BeanType.SERVICE);
			if (isMethodExists(service, methodName, paramType)) {
			...
			Object mapper = getBean(serviceName, BeanType.MAPPER);
			if (MethodType.SELECT.equals(prefix) && isMethodExists(mapper, methodName, paramType)) {
				String mapperName = getBeanName(serviceName, BeanType.MAPPER);
